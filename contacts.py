...
class Contact_book:
    ...
    def correct_names_and_phones(self):
        '''
        Формирование списка контактов с исправленными именами, фамилиями и телефонами
        :return: список с исправленными именами, фамилиями и телефонами
        '''
    ...
    def _entries_dict(self):
        '''
        Формирование словаря, хранящего данные об именах всех полей и их значениях в разных записях
        :return: словарь, где:
            - ключ: заголовок поля (элемент первой строки исходного файла),
            - значение: список значений соответствующего поля из всех записей
        '''
    ...
    def _header_index_entry(self):
        '''
        Формирование словаря полей и их значений в каждой из записей
        :return: словарь, где:
                - ключ: заголовок поля (элемент первой строки исходного файла),
                - значение: словарь, где:
                    - ключ: индекс записи в списке всех записей,
                    - значение: значение соответствующего поля из соответствующей записи
        '''
    ...
    def _dupl_dict(self, *fields):
        '''
        Формирование словаря дублирующихся записей
        :param fields: заголовки полей (если ни один заголовок не передан, обрабатываются все заголовки)
        :return: словарь дублируюихся записей, где:
            - ключ: заголовок поля (элемент первой строки исходного файла),
            - значение: словарь, где:
                - значение: список индексов записей, в которых поле, являющееся ключом внешнего словаря,
                  имеет одно и то же значение
                - ключ: первый  элемент из списка, являющегося значением
        '''
    ...
    def _dupl_bool(self, *fields):
        '''
        Проверка на наличие записей с одинаковыми значениями полей, принимаемых в качестве аргументов
        :param fields: заголовки полей (если ни один заголовок не передан, обрабатываются все заголовки)
        :return: кортеж из двух элементов, где:
            ...
            - второй элемент - словарь, где:
                ...
                - значение: список индексов записей с одинаковым значением в соответствующем поле,
    ...
    def _entries_numbers_delition_list(self, *fields):
        '''
        Проверка: для всех ли заголовков полей найдены записи с повторяющимися значениями в соответствующем поле
        :param fields: заголовки полей (если ни один заголовок не передан, обрабатываются все заголовки)
        :return: возвращаемое значение зависит от результатов проверки:
            - при отрицательном результате возвращается f-строка с соответствующим предупреждением
            - при положительном результате возвращается список списков, где:
    ...
    def _delition_list(self, *fields):
        '''
        Формирование словаря с данными о полях и индексах записей для удаления
        :param fields: заголовки полей (если ни один заголовок не передан, обрабатываются все заголовки)
        :return: список списков, где:
    ...
    def _clear_dict(self, *fields):
        '''
        Формирование словаря данных, где удалены дублирующиеся записи
        :param fields: Параметры: заголовки полей (если ни один заголовок не передан, обрабатываются все заголовки)
        :return: словарь, где:
    ...
    def del_dupls(self, *fields):
        '''
        Формирование списка очищенных данных
        :param fields: заголовки полей (если ни один заголовок не передан, обрабатываются все заголовки)
        :return: список списков,где:
    ...
    def write_in_file(self, name_of_final_file, list_to_be_written):
        '''
        Запись списка, принятого в качестве аргумента, в scv-файл
        :param name_of_final_file:   имя истогового файла
        :param list_to_be_written: список, данные из которого записываются в итоговый файл
        :return: None
        '''
